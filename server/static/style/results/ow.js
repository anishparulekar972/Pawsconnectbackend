var OwUtils = function(){
    ...
    {   
        $(".floatbox_container").removeClass("blockuserprofile") // 30-12-2019
        $(".floatbox_container").addClass("flagprofile") // 27-12-2019
...
function OW_FloatBox(options)
{
    ...
    {   
        if ( $(e.target).is(this) )
        {   
            // 27-12-2019 sign in popup disable body click    
            if($( "body" ).hasClass( "not_sign_in" )){
                return false;
            }
            // End
            fl_box.close({
    ...
    jQuery('.close', this.$header)
        ...
        {   
            // 31-12-2019 start
            if($( ".floatbox_container" ).hasClass( "ajax_signin" )){
                $(".floatbox_container").removeClass('ajax_signin');
            }
            // end
    ...
    $preview.find("span").click(function(e){
        ...
        if (!$("#" + id + "_preload_avatar").length) {
            $.ajax({
                ...
                dataType: 'json',
                success: function(data){ $(".profileimage").removeClass('ow_hidden'); $(".upload-btn-wrapper .btn").show(); }
            });
        }
...
OW_UsersApi = function( _settings )
{
    ...
    this.blockUserWithConfirmation = function( userId, confirmationCallback, callback ) {
        $(".floatbox_container").removeClass("flagprofile") // 30-12-2019
        $(".floatbox_container").addClass("blockuserprofile") // 27-12-2019
