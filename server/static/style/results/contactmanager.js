...
MAILBOX_Contact = Backbone.Model.extend({
    defaults: {
        ...
        isFriend: false,
        isGoldenMessage: false,
        isMyGoldenMessage: false,
...
MAILBOX_ContactView = Backbone.View.extend({
    ...
    initialize: function(){
        ...
        this.chatItemPhotoWrap = $('span.ow_chat_item_photo_wrap', this.$el);
        ...
        this.model.on('change:unreadMessagesCount', function(){
            ...
            if (this.model.get('unreadMessagesCount') > 0){
                ...
                if (this.model.get('isGoldenMessage') === true) {
                    this.chatItemPhotoWrap.addClass('gm_sender_border');
                }
            }
            else{
                ...
                if (this.model.get('isGoldenMessage') === true) {
//                    this.chatItemPhotoWrap.removeClass('gm_sender_border');
                }
            }
        }, this);
        ...
        this.$el.click(function (){
            if (typeof OW.Mailbox.newMessageFormController != "undefined") {
                if (OW.Mailbox.newMessageFormController.closeNewMessageWindowWithConfirmation($('.mailboxDialogBlock.ow_open').length)) {
                    OW.trigger('mailbox.open_dialog', {convId: self.model.get('convId'), opponentId: self.model.get('opponentId'), mode: 'chat', isSelected: true, isActive: true, isFriend: self.model.get('isFriend'), isGoldenMessage: self.model.get('isGoldenMessage'), isMyGoldenMessage: self.model.get('isMyGoldenMessage')});
                }  
            }
            else {
                OW.trigger('mailbox.open_dialog', {convId: self.model.get('convId'), opponentId: self.model.get('opponentId'), mode: 'chat', isSelected: true, isActive: true, isFriend: self.model.get('isFriend'), isGoldenMessage: self.model.get('isGoldenMessage'), isMyGoldenMessage: self.model.get('isMyGoldenMessage')});
                ...
                if( message.senderId == OWMailbox.userDetails.userId ){
                    ...
                    if (dialog){
                        if (!message.conversationViewed && !conversationOpened && message.mode == 'chat' && dialog.model.status != 'offline' && !dialog.model.isOpened && !dialog.model.isActive){
                            //dialog.showTab().open(); - 27/01/2020 
        ...
        if (OWMailbox.chatModeEnabled && OWMailbox.useChat == 'available'){
            ...
            OW.bind('mailbox.application_started', function(){
                ...
                $.each(openedDialogs, function(convId, presence){
                    ...
                    {
                        OW.trigger('mailbox.open_dialog', {convId: convId, opponentId: presence.opponentId, mode: presence.mode, isActive: presence.isActive, isSelected: presence.isSelected, isFriend: dialog.model.isFriend, isGoldenMessage: dialog.model.isGoldenMessage, isMyGoldenMessage: dialog.model.isMyGoldenMessage});
                    }
                });
            ...
            OW.bind('mailbox.open_dialog', function(data){
                ...
                if (data.hasOwnProperty('isFriend')) {
                    if (data['isFriend'] === false) {
                        dialog.messageFormBlock[0].style.display = 'none';
                    } else {
                        dialog.messageFormBlock[0].style.display = 'block';
                    }
                }
                if (data.hasOwnProperty('isMyGoldenMessage')) {
                    if (data['isMyGoldenMessage'] === false) {
                        dialog.messageGroupStickyGoldenMessageBlockControl.remove();
                    }
                }
...
//            this.model.set('soundEnabled', OWMailbox.soundEnabled);
    ...
    getDialog: function(convId, opponentId){
        ...
        if( typeof self.dialogs[convId] == 'undefined' ){
            ...
            dialogModel.setIsFriend(contactModel.get('isFriend'));
            dialogModel.setIsGoldenMessage(contactModel.get('isGoldenMessage'));
            dialogModel.setIsMyGoldenMessage(contactModel.get('isMyGoldenMessage'));
...
OWMailbox.Dialog.Model = function(convId){
    ...
    this.isOpened = false;
    this.isFriend = true;
    this.isGoldenMessage = false;
    this.isMyGoldenMessage = false;
...
OWMailbox.Dialog.Model.prototype = {
    ...
    setIsGoldenMessage: function(value){
        this.isGoldenMessage = value;
        this.isActiveSetSubject.notifyObservers();
    },
    setIsMyGoldenMessage: function(value){
        this.isMyGoldenMessage = value;
        this.isActiveSetSubject.notifyObservers();
    },
...
//        $('input', self.attachmentsBtn).removeAttr('disabled');
    ...
    this.smallItemControl.click(function(){
        if (typeof OW.Mailbox.newMessageFormController != "undefined") {
            if (OW.Mailbox.newMessageFormController.closeNewMessageWindowWithConfirmation($('.mailboxDialogBlock.ow_open').length)) {
                OW.trigger('mailbox.open_dialog', {convId: self.model.convId, opponentId: self.model.convId, mode: self.model.mode, isFriend: self.model.isFriend, isGoldenMessage: self.model.isGoldenMessage, isMyGoldenMessage: self.model.isMyGoldenMessage});
            }
        }
        else {
            OW.trigger('mailbox.open_dialog', {convId: self.model.convId, opponentId: self.model.convId, mode: self.model.mode, isFriend: self.model.isFriend, isGoldenMessage: self.model.isGoldenMessage, isMyGoldenMessage: self.model.isMyGoldenMessage});
...
//        }
    ...
    this.model.isActiveSetSubject.addObserver(function(){
        if (!self.model.isActive){
            ...
            if (self.model.unreadMessagesCount > 0){
                ...
                if (self.model['isGoldenMessage'] === true) {
                    self.profileUrlControl.addClass('gm_sender_border');
                }
    ...
    this.model.isOpenedSetSubject.addObserver(function(){
        ...
        if (self.model.hasOwnProperty('isFriend')) {
            if (self.model['isFriend'] === false) {
                self.messageFormBlock[0].style.display = 'none';
            } else {
                self.messageFormBlock[0].style.display = 'block';
            }
        }
        if (self.model.hasOwnProperty('isMyGoldenMessage')) {
            if (self.model['isMyGoldenMessage'] === false) {
                self.messageGroupStickyGoldenMessageBlockControl.remove();
            }
        }

        if (self.model.convId != 0 && typeof openedDialogs[self.model.convId] == 'undefined'){
            openedDialogs[self.model.convId] = {opponentId: self.model.opponentId, mode: self.model.mode, isActive: self.model.isActive, isSelected: self.model.isSelected, isFriend: self.model.isFriend, isGoldenMessage: self.model.isGoldenMessage, isMyGoldenMessage: self.model.isMyGoldenMessage};
        }
...
//        this.subjectBlockControl = $('#dialogSubjectBlock', this.control);
        ...
        this.messageGroupStickyGoldenMessageBlockControl = $('#dialogStickyGoldenMessageBlock', this.control);
...
//        this.model.setIsSelected(true);
    ...
    setData: function(){
        ...
        {
            this.model.setIsFriend(this.model.isFriend);
        }
        if (this.model.isGoldenMessage)
        {
            this.model.setIsGoldenMessage(this.model.isGoldenMessage);
        }
        if (this.model.isMyGoldenMessage)
        {
            this.model.setIsMyGoldenMessage(this.model.isMyGoldenMessage);
        }
